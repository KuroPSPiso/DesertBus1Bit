...
// set position of overlayed window
void vm_overlay_setpos_px(SCRIPT_CTX* THIS, UBYTE pos_x, UBYTE pos_y) OLDCALL BANKED {
    THIS;
    ui_set_pos(pos_x, pos_y);
    //ui_set_pos(pos_x << 3, pos_y << 3);
}
...
void vm_overlay_move_to_px(SCRIPT_CTX* THIS, UBYTE pos_x, UBYTE pos_y, BYTE speed) OLDCALL BANKED {
    THIS;
    if (speed == UI_IN_SPEED) {
        speed = text_in_speed;
    }
